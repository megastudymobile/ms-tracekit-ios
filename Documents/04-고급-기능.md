...
> 💡 **v1.2.1+ 새 기능**: Variadic Parameters를 사용한 간결한 metadata API  
> 이 문서의 예제는 새로운 API(`("key", value)`)를 사용합니다.  
> 기존 dictionary 방식(`metadata: ["key": AnyCodable(value)]`)도 여전히 지원됩니다.

> 💡 일반 로깅은 동기 API(`TraceKit.info(...)`)를 사용하고,  
...
// ✨ v1.2.1+ Variadic Parameters API로 마스킹 (권장)
...
TraceKit.info(
    "결제 정보",
    category: "Payment",
    ("cardNumber", "1234-5678-9012-3456"),  // 자동 마스킹 → [CREDIT_CARD]
    ("email", "john@example.com")           // 자동 마스킹 → [EMAIL]
)
```
