# 📝 사용법

> 작성일: 2025-12-15
> 작성자: jimmy

## 기본 사용법

### 1. 가장 간단한 사용 (동기 API)

TraceKit는 `await` 없이도 사용할 수 있습니다:

```swift
import TraceKit

// await 없이 바로 사용 (Fire-and-Forget)
TraceKit.info("앱이 시작되었습니다")

// 인스턴스 메서드도 동일하게 사용
TraceKit.shared.info("앱이 시작되었습니다")
```

### 2. 레벨별 로깅

```swift
import TraceKit

// 동기 API (await 불필요) - 권장
TraceKit.verbose("함수 진입: loadData()")
TraceKit.debug("API 요청: GET /users")
TraceKit.info("사용자 로그인 성공")
TraceKit.warning("토큰 만료까지 5분 남음")
TraceKit.error("네트워크 연결 실패")
TraceKit.fatal("데이터베이스 연결 불가")

// 비동기 API (await 필요) - 로그 완료 대기 시 사용
Task {
    await TraceKit.shared.info("로그 완료까지 대기")
}
```

### 3. 동기 vs 비동기 API

| 상황 | 권장 API | 예시 |
|-----|---------|-----|
| 일반 로깅 | 동기 (await 없음) | `TraceKit.info("message")` |
| UI/이벤트 핸들러 | 동기 (await 없음) | `TraceKit.debug("button tapped")` |
| 로그 완료 대기 필요 | 비동기 (await) | `await TraceKit.shared.info("...")` |
| 테스트에서 검증 | 비동기 (await) | `await TraceKit.shared.flush()` |

### 4. 카테고리와 메타데이터

```swift
import TraceKit

// 동기 API로 카테고리 지정
TraceKit.info("API 요청 시작", category: "Network")

// 메타데이터 추가
TraceKit.info(
    "사용자 로그인",
    category: "Auth",
    metadata: [
        "userId": AnyCodable("user123"),
        "method": AnyCodable("OAuth"),
        "provider": AnyCodable("Google")
    ]
)
```

## TraceKit 설정

### TraceKitBuilder 사용

```swift
import TraceKit

@main
struct MyApp: App {
    init() {
        Task {
            await setupTraceKit()
        }
    }
    
    @TraceKitActor
    func setupTraceKit() async {
        let logger = await TraceKitBuilder()
            // Destinations
            .addConsole(
                minLevel: .verbose,
                formatter: PrettyTraceFormatter.verbose
            )
            .addOSLog(
                subsystem: "com.myapp",
                minLevel: .info
            )
            .addFile(
                minLevel: .info,
                retentionPolicy: .default
            )
            // 정책
            .withBuffer(policy: .default)
            .withSampling(policy: .production)
            // 민감정보 마스킹
            .withDefaultSanitizer()
            // 사용자 컨텍스트
            .withDefaultContextProvider(environment: .production)
            // Launch Argument 적용
            .applyLaunchArguments()
            // 공유 인스턴스로 빌드
            .buildAsShared()
    }
    
    var body: some Scene {
        WindowGroup {
            ContentView()
        }
    }
}
```

### 프리셋 사용

```swift
// 디버그용 (모든 로그, 상세 출력)
let debugTraceKit = await TraceKitBuilder.debug().buildAsShared()

// 프로덕션용 (최적화된 설정)
let prodTraceKit = await TraceKitBuilder.production().buildAsShared()
```

### 디버그 프리셋 구성

```swift
// TraceKitBuilder.debug()는 내부적으로 다음과 같이 구성됩니다
TraceKitBuilder()
    .addConsole(formatter: PrettyTraceFormatter.verbose)
    .with(configuration: .debug)
    .withDefaultSanitizer()
    .applyLaunchArguments()
```

### 프로덕션 프리셋 구성

```swift
// TraceKitBuilder.production()은 내부적으로 다음과 같이 구성됩니다
TraceKitBuilder()
    .addConsole(minLevel: .warning)
    .addOSLog(minLevel: .info)
    .addFile(minLevel: .info)
    .with(configuration: .production)
    .withBuffer(policy: .default)
    .withSampling(policy: .production)
    .withDefaultSanitizer()
    .withCrashPreservation()
    .applyLaunchArguments()
```

## Formatter

### PrettyTraceFormatter (콘솔용)

```swift
// 최소 출력 (레벨 + 메시지만)
let minimal = PrettyTraceFormatter.minimal
// 출력: "ℹ️ INFO 앱이 시작되었습니다"

// 표준 출력 (기본값)
let standard = PrettyTraceFormatter.standard
// 출력: "14:32:15.123 ℹ️ INFO [Default] 앱이 시작되었습니다 (App.swift:42)"

// 상세 출력 (모든 정보)
let verbose = PrettyTraceFormatter.verbose
// 출력: "14:32:15.123 ℹ️ INFO [Auth] 사용자 로그인 (AuthManager.swift:123) {userId=user123, method=OAuth}"
```

커스텀 설정:

```swift
let customFormatter = PrettyTraceFormatter(
    includeTimestamp: true,     // 타임스탬프 포함
    includeLocation: true,      // 파일:라인 포함
    includeMetadata: false,     // 메타데이터 제외
    useEmoji: false             // 이모지 대신 [LEVEL]
)
```

### JSONTraceFormatter (파일/분석용)

```swift
let jsonFormatter = JSONTraceFormatter()
// 출력: {"category":"Auth","file":"AuthManager.swift",...}

let prettyJsonFormatter = JSONTraceFormatter(prettyPrint: true)
// 출력:
// {
//   "category": "Auth",
//   "file": "AuthManager.swift",
//   ...
// }
```

## 출력 대상 (Destinations)

### ConsoleTraceDestination

```swift
// 기본 사용
let console = ConsoleTraceDestination()

// 커스텀 설정
let console = ConsoleTraceDestination(
    minLevel: .debug,
    isEnabled: true,
    formatter: PrettyTraceFormatter.verbose,
    outputStream: .auto  // error/fatal은 stderr, 나머지는 stdout
)
```

### OSTraceDestination (iOS 14+)

```swift
let oslog = OSTraceDestination(
    subsystem: Bundle.main.bundleIdentifier ?? "com.myapp",
    minLevel: .info,
    isEnabled: true
)

// OSLog 레벨 매핑
// verbose, debug → OSLogType.debug
// info → OSLogType.info
// warning → OSLogType.default
// error → OSLogType.error
// fatal → OSLogType.fault
```

### FileTraceDestination

```swift
let fileManager = TraceFileManager(
    retentionPolicy: TraceFileRetentionPolicy(
        maxFileSize: 10 * 1024 * 1024,  // 10MB
        maxFileCount: 5,
        maxAge: 7 * 24 * 3600           // 7일
    )
)

let file = FileTraceDestination(
    minLevel: .info,
    isEnabled: true,
    formatter: JSONTraceFormatter(),
    fileManager: fileManager,
    bufferSize: 10  // 10개 로그 후 파일 쓰기
)
```

## Configuration

### TraceKitConfiguration

```swift
// 직접 생성
let config = TraceKitConfiguration(
    minLevel: .info,
    enabledCategories: ["Network", "Auth"],  // nil이면 모든 카테고리
    disabledDestinations: ["file"],          // 비활성화할 destination
    isSanitizingEnabled: true,               // 민감정보 마스킹
    sampleRate: 0.5,                         // 50% 샘플링
    bufferSize: 100,
    flushInterval: 5.0
)

// 프리셋
let debug = TraceKitConfiguration.debug       // 모든 로그, 마스킹 비활성화
let production = TraceKitConfiguration.production  // info 이상, 10% 샘플링
```

### 런타임 설정 변경

```swift
Task {
    // 현재 설정 변경
    await TraceKit.shared.configure(
        TraceKitConfiguration(minLevel: .warning)
    )
}
```

## 실전 예제

### ViewModel에서 사용

```swift
@MainActor
final class UserViewModel: ObservableObject {
    @Published var user: User?
    @Published var error: Error?
    
    func login(email: String, password: String) async {
        // 동기 API 사용 (await 불필요)
        TraceKit.info(
            "로그인 시도",
            category: "Auth",
            metadata: ["email": AnyCodable(email)]  // 이메일은 자동 마스킹됨
        )
        
        do {
            user = try await authService.login(email: email, password: password)
            TraceKit.info("로그인 성공", category: "Auth")
        } catch {
            self.error = error
            TraceKit.error(
                "로그인 실패: \(error.localizedDescription)",
                category: "Auth"
            )
        }
    }
}
```

### 네트워크 레이어에서 사용

```swift
actor NetworkService {
    func request<T: Decodable>(_ endpoint: Endpoint) async throws -> T {
        // 성능 추적은 비동기 API 사용 (span ID 반환 필요)
        let spanId = await TraceKit.shared.startSpan(name: "API: \(endpoint.path)")
        
        // 일반 로깅은 동기 API 사용
        TraceKit.debug(
            "API 요청",
            category: "Network",
            metadata: [
                "method": AnyCodable(endpoint.method.rawValue),
                "path": AnyCodable(endpoint.path)
            ]
        )
        
        do {
            let (data, response) = try await urlSession.data(for: endpoint.urlRequest)
            
            guard let httpResponse = response as? HTTPURLResponse else {
                throw NetworkError.invalidResponse
            }
            
            TraceKit.debug(
                "API 응답",
                category: "Network",
                metadata: [
                    "statusCode": AnyCodable(httpResponse.statusCode),
                    "dataSize": AnyCodable(data.count)
                ]
            )
            
            let result = try decoder.decode(T.self, from: data)
            
            await TraceKit.shared.endSpan(id: spanId)
            return result
            
        } catch {
            TraceKit.error(
                "API 에러: \(error.localizedDescription)",
                category: "Network"
            )
            await TraceKit.shared.endSpan(id: spanId)
            throw error
        }
    }
}
```

### Repository에서 사용

```swift
actor UserRepository {
    private let networkService: NetworkService
    private let cache: UserCache
    
    func getUser(id: String) async throws -> User {
        // 동기 API 사용
        TraceKit.verbose("getUser 호출", category: "Repository")
        
        // 캐시 확인
        if let cached = await cache.get(id: id) {
            TraceKit.debug("캐시 히트", category: "Repository")
            return cached
        }
        
        TraceKit.debug("캐시 미스, API 호출", category: "Repository")
        
        let user: User = try await networkService.request(.user(id: id))
        await cache.set(user)
        
        return user
    }
}
```

## 다음 단계

- [고급 기능](./04-고급-기능.md) - 버퍼링, 샘플링, 마스킹 상세
- [외부 연동](./05-외부-연동.md) - Sentry, Datadog, Firebase 연동

